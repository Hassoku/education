...
class LearningSessionRequestController extends Controller
{
    ...
    public function send(Request $request, $learning_session_request_id){
        ...
        if($tutor->online_status == 1){
            ...
            if(!$tutor->isBusy){
                ...
                return ['status' => 'success', 'msg' => 'Request success fully sent',
            ...
            return ['status'=> 'error', 'online_status' => 0,'msg' => 'Tutor is offline'];
        }
    }
    ...
    public function withdraw($learning_session_request_id){
        ...
        CommonHelper::puhser()->trigger('' . $channel,''.$event, $data);
        return ['status'=>'success','msg' => 'request withdrew'];
    }
}